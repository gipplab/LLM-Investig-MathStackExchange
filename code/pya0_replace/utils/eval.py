...
import collection_driver_local
import tracemalloc


def gen_topics_queries(collection, qfilter=None):
    func_name = '_topic_process__' + collection.replace('-', '_')
    print("eval.gen_topics_queries. func_name: ",func_name)
    handler = getattr(collection_driver_local, func_name)
    cache = get_cache_home()
    curdir = os.path.dirname(os.path.abspath(__file__))
    #prefix = f'{curdir}/topics-and-qrels/topics.{collection}'
    prefix = f'{os.path.dirname(curdir)}/topics-and-qrels/topics.{collection}'
    print(f'Searching topics file at: {prefix} ...')
    found = False
    for src in [f'{prefix}.{ent}' for ent in ['txt', 'json', 'xml','csv']]:
        ...
        if ext == 'txt':
            ...
            import csv
            import replace_post_tex_local
            with open(src,'r',encoding='utf-8') as fh:
                qlist = csv.reader(fh,delimiter = "\t")
                for i, line in enumerate(qlist):
                    if len(line) == 2:
                        qid,answer = line
                    else:
                        qid,query,answer = line
                        answer = " ".join([query,answer])
                    answer = replace_post_tex_local.replace_display_tex(answer)
                    answer = replace_post_tex_local.replace_inline_tex(answer)
                    answer = replace_post_tex_local.replace_dollar_tex(answer)
                    answer = replace_post_tex_local.unwrap_tex_groups(answer)
                    answer = [{'type': 'term', 'str': answer}]
                    print(qid)
                    yield qid,answer, None
    if not found:
...
def run_fold_topics(index, collection, k, fold, cascades, output, topk, purpose,
    ...
    for topic_query_ in fold:
        ...
        if output is not None:
            collection_driver_local.TREC_preprocess(collection, index, hits)
