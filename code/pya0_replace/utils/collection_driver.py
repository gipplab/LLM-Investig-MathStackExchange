...
def TREC_preprocess(collection, index, hits):
    if collection in ['test', 'arqmath-2020-task1', 'arqmath-2021-task1', 'arqmath-2021-task1-refined', 'arqmath-2020-task1-origin', 'arqmath-2021-task1-origin', 'arqmath-2022-task1-manual', 'arqmath-2022-task1-or-task3-origin']:
        for hit in hits:
...
def _topic_process__arqmath_2022_generated_answers():
    return

def _topic_process__arqmath_2022_origin_and_generated_answers():
    return

def _topic_process__arqmath_2022_llemma_7b_generated_answers():
    return

def _topic_process__arqmath_2022_llemma_7b_origin_and_generated_answers():
    return


def _topic_process__arqmath_2022_MAmmoTH_Coder_7b_generated_answers():
    return

def _topic_process__arqmath_2022_MAmmoTH_Coder_7b_origin_and_generated_answers():
    return

def _topic_process__arqmath_2022_gpt4_generated_answers():
    return

def _topic_process__arqmath_2022_tora_code_13b_generated_answers():
    return

def _topic_process__arqmath_2022_tora_code_7b_generated_answers():
    return

def _topic_process__arqmath_2022_Mistral_7B_Instruct_generated_answers():
    return
...
def _topic_process__plag_dataset(xmlfile):
    from xmlr import xmliter
    from bs4 import BeautifulSoup
    import replace_post_tex
    from preprocess import unwrap_isolated_tex_groups
    for attrs in xmliter(xmlfile, 'doc'):
        qid = attrs['id']
        post = replace_post_tex.replace_dollar_tex(attrs['body'])
...
def _topic_process__arqmath_2020_task1_origin(xmlfile, llm=False):
    from xmlr import xmliter
    from bs4 import BeautifulSoup
    import replace_post_tex
    from preprocess import unwrap_isolated_tex_groups
    ...
    for attrs in xmliter(xmlfile, 'Topic'):
        ...
        post = unwrap_isolated_tex_groups(post)
        if llm == True:
            query = ["Represent this query for retrieving relevant documents: " + post]
        else:
            query = [{
                'type': 'term',
                'str': post
...
def _topic_process__arqmath_2022_task1_or_task3_origin(xmlfile):
    for qid, query, _ in _topic_process__arqmath_2020_task1_origin(xmlfile):
        yield qid, query, None
...
def _topic_process__arqmath_2022_task2_context(idx, json_item):
    qid = json_item['qid']
    query = [{
        'type': 'term',
